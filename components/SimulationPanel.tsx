...
import { SimulationParams, DeviceConfig } from '../types';
import { Settings, Save, Wand2, Info, CheckCircle } from 'lucide-react';
...
const SimulationPanel: React.FC<SimulationPanelProps> = ({ 
  ...
  const handleDeviceChange = (id: number, field: keyof DeviceConfig, value: string) => {
    ...
    const newDevices = params.devices.map(d => {
      if (d.id === id) {
        return { ...d, [field]: numValue };
      }
      return d;
    });
    onChange({ ...params, devices: newDevices });
  };
    ...
    <div className="bg-white w-full lg:w-96 p-6 shadow-sm border-l border-slate-200 flex flex-col h-full overflow-y-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-700">Line Configuration</h3>
          <div className="group relative">
             ...
             <div className="hidden group-hover:block absolute right-0 w-48 bg-slate-800 text-white text-xs p-2 rounded z-10">
               Modify per-device parameters to simulate capacity changes.
             </div>
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-2 py-2 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Maintenance Days">MNT(Day)</th>
                <th className="px-2 py-2 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Weekend Overtime Days">Weekend OT(Day)</th>
              </tr>
            </thead>
        ...
        <p className="text-xs text-slate-400 mt-2">
           Values apply to the upcoming N+1 month simulation.
        </p>
      </div>
